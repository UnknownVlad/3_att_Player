...
public class ItemServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            rs = DataBaseManager.getResultSet(Requests.read(Requests.ITEM.tableName(), collumns, values), data.getConnection());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (collumns.size() != Requests.ITEM.collumns().size()){
            response.getWriter().println(
                    "you haven't filled in all the fields\n" +
                            "fill all: "+Requests.ITEM.collumns());
            return;
        }
        ...
        try {
            DataBaseManager.executeUpdate(Requests.insert(Requests.ITEM.tableName(), collumns, values), data.getConnection());
        } catch (SQLException e) {
            response.getWriter().println("This item already exist");
        }
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            DataBaseManager.executeUpdate(Requests.remove(Requests.ITEM.tableName(), collumns, values), data.getConnection());
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            DataBaseManager.executeUpdate(Requests.update(Requests.ITEM.tableName(), collumns2, values2, collumns1, values1), data.getConnection());
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
